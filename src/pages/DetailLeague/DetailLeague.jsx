...
const DetailLeague = () => {
  ...
  useEffect(() => {
    fetch(
      `https://www.thesportsdb.com/api/v1/json/3/search_all_teams.php?l=${id}`
    )
      .then((res) => res.json())
  ...
  if (myLeague && myLeague.teams && myLeague.teams.length > 0) {
    ...
    const sportImage = sportImageSourcePath.find(
      (sport) => sport.strSport === sportVar
    );
    detailLeagueImageSource = sportImage
      ? sportImage.strSportThumb
      : "public/img/detail-league/sports.jpg";
  } else {
        ...
        <div className="container montserrat">
          <header className="detailLeagueHeader">
            <div className="headerContainerLeft">
              <img
                src={detailLeagueImageSource}
                alt="sports image"
                className="detailLeagueHeaderImage"
              />
            </div>
              ...
              {myLeague.teams !== null ? (
                <div className="detailLeagueHeadingContainer">
                  ...
                  <h1 className="detailLeagueHeading">
                    {myLeague.teams[0]
                      ? myLeague.teams[0].strLeague
                      : myLeague.teams.strLeague}
                  </h1>
                  {/* <p>{myLeague.teams[0].strSport}</p> */}
                  <p>
                    {myLeague.teams[0]
                      ? myLeague.teams[0].strSport
                      : myLeague.teams.strSport}
                  </p>
                </div>
              ) : (
              ...
              {myLeague.teams.map((element) => {
                ...
                return (
                  <div className="detailLeagueInnerOutput" key={element.idTeam}>
                    <Link
                      className="detailLeagueLink"
                      to={`/detailteams/${element.strTeam}`}
                    >
