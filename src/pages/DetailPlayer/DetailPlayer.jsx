...
const DetailPlayer = () => {
  ...
  console.log("Params aus der URL:", params);
  const { id } = params;
  ...
  useEffect(() => {
    if (id) {
        fetch(`https://www.thesportsdb.com/api/v1/json/60130162/searchplayers.php?p=${id}`)
            .then((res) => {
                if (!res.ok) {
            ...
            <>
                ...
                <div className="header-flex">
                    <div className="player-infos">
                        ...
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strNationality}</p>
                          <p className="detail-text opensans">Nationality</p>
                        </>
                        }
                        {players[0]?.strGender && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strGender}</p>
                          <p className="detail-text opensans">Gender</p>
                        </>
                        }
                        {players[0]?.strBirthLocation && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strBirthLocation}</p>
                          <p className="detail-text opensans">Birth Location</p>
                        </>
                        }
                        {players[0]?.dateBorn && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.dateBorn}</p>
                          <p className="detail-text opensans">Date of Birth</p>
                        </>
                        }
                        {players[0]?.strSport && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strSport}</p>
                          <p className="detail-text opensans">Sport</p>
                        </>
                        }
                        {players[0]?.strPosition && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strPosition}</p>
                          <p className="detail-text opensans">Position</p>
                        </>
                        }
                        {players[0]?.strStatus && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strStatus}</p>
                          <p className="detail-text opensans">Status</p>
                        </>
                        }
                        {players[0]?.strHeight && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strHeight}</p>
                          <p className="detail-text opensans">Height</p>
                        </>
                        }
                        {players[0]?.strWeight && 
                        <>
                          <p className="detail-headline montserrat">{players[0]?.strWeight}</p>
                    ...
                    <div className="player-img">
                        ...
                        <>
                          ...
                          <p className="player-details montserrat">{players[0]?.strDescriptionEN}</p>
                        </>
                        }
                        <div className="player-socials opensans">
                          {players[0]?.strWebsite &&
                            <a className="opensans" target="_blank" href={"https://" + players[0]?.strWebsite}>
                                WEBSITE
                            </a>
                          }
                          {players[0]?.strFacebook &&
                            <a className="opensans" target="_blank" href={"https://" + players[0]?.strFacebook}>
                                FACEBOOK
                            </a>
                          }
                          {players[0]?.strTwitter &&
                            <a className="opensans" target="_blank" href={"https://" + players[0]?.strTwitter}>
                                TWITTER
                            </a>
                          }
                          {players[0]?.strInstagram &&
                            <a className="opensans" target="_blank" href={"https://" + players[0]?.strInstagram}>
                                INSTAGRAM
                            </a>
                          }
                          {players[0]?.strYoutube &&
                            <a className="opensans" target="_blank" href={"https://" + players[0]?.strYoutube}>
                                YOUTUBE
                            </a>
                          }
                      </div>
